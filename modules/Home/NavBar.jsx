  ...
  return (
    <Box className="nav">
      <Box className="nav-header">
        <Box className="nav-title">H3Trivia.com</Box>
      </Box>

      <Box className="nav-links">
        <Box>Top Win Streaks: {topScore ? `${topScore} rounds` : null}</Box>
        <Box>Games Played: {totalGames}</Box>
        <Box
          _after={{
            content: '""',
            position: "absolute",
        ...
        >
          Win Rate: {isNaN(winRate) ? null : `${winRate}%`}
        </Box>
      </Box>
    </Box>
  );
